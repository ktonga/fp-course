...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  (<$>) f pa = P ((`onResult` p) . parse pa)
    where p i = Result i . f
...
(|||) p1 p2 = P p
  where p i = let r = parse p1 i in if isErrorResult r then parse p2 i else r
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser b
  (=<<) f pa = P pb
   where pb = flip onResult (flip $ parse . f) . parse pa
...
satisfy f = g =<< character
  where g c = if f c then pure c else unexpectedCharParser c
...
list pa = go Nil
  where go as = ((\a -> go (as ++ (a :. Nil))) =<< pa) ||| pure as
...
surnameParser = (++) <$> mandatory <*> list lower
  where mandatory = (:.) <$> upper <*> thisMany 5 lower
