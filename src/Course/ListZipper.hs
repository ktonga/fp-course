...
moveLeftN n lz = if n < 0 then moveRightN (-n) lz else go n (IsZ lz)
 where
  go 0  mlz       = mlz
  go n' (IsZ lz') = go (n' - 1) (moveLeft lz')
  go _  IsNotZ    = IsNotZ
...
moveRightN n lz = if n < 0 then moveLeftN (-n) lz else go n (IsZ lz)
 where
  go 0  mlz       = mlz
  go n' (IsZ lz') = go (n' - 1) (moveRight lz')
  go _  IsNotZ    = IsNotZ
...
end lz@(ListZipper l a r) = case reverse r of
  Nil      -> lz
  a' :. l' -> ListZipper (l' ++ a :. l) a' Nil
...
start lz@(ListZipper l a r) = case reverse l of
  Nil      -> lz
  a' :. r' -> ListZipper Nil a' (r' ++ a :. r)
...
-- /Tip:/ Use @List#repeat@.
  pure a = let as = repeat a in ListZipper as a as
-- /Tip:/ Use `zipWith`
...
instance Extend ListZipper where
  (<<=) f lz = ListZipper l (f lz) r
   where
     f' lz' = (f lz', lz')
     l = unfoldr ((f' <$>) . toOptional . moveLeft) lz
     r = unfoldr ((f' <$>) . toOptional . moveRight) lz
