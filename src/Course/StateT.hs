...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<*>) stf sta = StateT $ ((\(f, s) -> runStateT (f <$> sta) s) =<<) . runStateT stf
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (=<<) f sta = StateT $ ((\(a, s) -> runStateT (f a) s) =<<) . runStateT sta
...
distinct' as =
  let pM a = state' (\s -> (not $ a `S.member` s, a `S.insert` s))
  in  eval' (filtering pM as) S.empty
...
distinctF as =
  let pM a = if a > 100
        ...
        else StateT (\s -> Full (not $ a `S.member` s, a `S.insert` s))
  in  evalT (filtering pM as) S.empty
...
instance Monad f => Applicative (OptionalT f) where
  ...
  (<*>) ::
    ...
    -> OptionalT f a
    -> OptionalT f b
  (<*>) otf ota = OptionalT $ do
    f <- runOptionalT otf
    a <- onFull (const $ runOptionalT ota) f
    pure $ f <*> a
...
instance Monad f => Monad (OptionalT f) where
  (=<<) ::
    ...
    -> OptionalT f a
    -> OptionalT f b
  (=<<) f ota = OptionalT $ onFull (runOptionalT . f) =<< runOptionalT ota
...
type SOL s a = StateT s (OptionalT (Logger Chars)) a

distinctG :: (Integral a, Show a) => List a -> Logger Chars (Optional (List a))
distinctG as =
  let -- log1' :: Chars -> SOL (S.Set a) ()
      log1' l = StateT (\s -> (, s) <$> (OptionalT . log1 l . Full $ ()))
      -- empty' :: SOL (S.Set a) ()
      empty' = StateT (\s -> (, s) <$> (OptionalT $ pure Empty))
      -- when :: Applicative f => Bool -> f () -> f ()
      when b f = if b then f else pure ()
      -- pM :: a -> SOL (S.Set a) Bool
      pM a = do
        when (a > 100) $ do
          log1' ("aborting > 100: " ++ show' a)
          empty'
        when (even a) $ log1' ("even number: " ++ show' a)
        StateT (\s -> pure (not $ a `S.member` s, a `S.insert` s))
                                                                                                                                                                                                                                                                                                          --   else StateT (\s -> Full (not $ a `S.member` s, a `S.insert` s))
  in  runOptionalT $ evalT (filtering pM as) S.empty

-- needs ScopedTypeVariables and forall
-- f' :: (Num a) => a -> a
-- f' = g
--  where
--   g :: a -> a
